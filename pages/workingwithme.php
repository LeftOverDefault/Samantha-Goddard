    <section class="working-with-me-hero">
        <div class="hero-text">
            <h1>Rapid Transformational Therapy</h1>
            <h4>Freedom from Anxiety, Low Self-Esteem, Weight & Eating Issues</h4>
        </div>
    </section>

    <section class="main">
        <div class="section">
            <h2>Working With Me</h2>
            <div class="text">
                <p>If you have spent years dieting and battling with unwanted weight only to find yourself back where
                    you started, then it’s time to try a different approach. To truly transform your relationship with
                    food and your body, you need to uncover the root cause that’s keeping you stuck. You hold the key to
                    unlocking your natural ability to enjoy a relaxed and healthy relationship with food and yourself.
                    The answers are all in your subconscious mind. I simply facilitate you in getting the understanding
                    and awareness. Once you are aware, you cannot help but change.</p>
                <p>Following your RTT session I send you a personalised, powerful hypnotic recording to wire in these
                    new empowering and motivating beliefs. The repetition of listening to your recording will help
                    cement these beliefs into your subconscious mind. Making healthy choices becomes your automatic
                    choice, exercise becomes something you enjoy and do willingly, feeling good about yourself is your
                    natural state of being.</p>
                <p>You have complete control the whole time. You can talk, move your body, get a tissue, even get up to
                    use the loo! If your session is over Zoom and our call gets disconnected you may drift into sleep if
                    you’re really relaxed, but eventually you’ll notice that you’re not hearing my voice and open your
                    eyes and reconnect our call.</p>
                <p>Hypnosis is not magic, it’s science. It’s very easy and anyone can do it. Please don’t worry about
                    whether you’re doing it right, just relax and know that it is working.</p>
                <p>If you’re ready to have a chat with me about what you’ve been struggling with and how I can help,
                    just click on the button below and book a free call. There’s no pressure to work with me or sign up,
                    it’s simply an informal chat for you to find out more and start to feel better.</p>
            </div>
            <div class="button-container">
                <a href="https://app.10to8.com/book/speaktosam/" target="_blank">
                    <button class="button">Book A Call</button>
                </a>
            </div>
        </div>
        <div class="section">
            <h2>Rapid Transformational Therapy</h2>
            <div class="text">
                <p>RTT&#174; is effective in treating:</p>
                <ul>
                    <li>Anxiety</li>
                    <li>Depression</li>
                    <li>Eating Issues</li>
                    <li>Weight Issues</li>
                    <li>Guilt</li>
                    <li>Stress</li>
                    <li>Self Esteem</li>
                    <li>Self Confidence</li>
                    <li>Sleep Problems</li>
                    <li>Fears & Phobias</li>
                    <li>Lack of Motivation</li>
                    <li>Relationships</li>
                    <li>Childhood Problems</li>
                </ul>
                <p>Sometimes it can take more than one session per issue but as a rule most issues only take one
                    session. *Please note long-term eating disorders, depression & anxiety can take several sessions.
                </p>
            </div>
        </div>
        <div class="section">
            <h2>Ways We Can Work Together</h2>
            <div class="card-wrapper">
                <div class="card">
                    <img src="../img/laptop.jpg" alt="Six Week Transformation, hypnotherapy, RTT, Samantha Goddard">
                    <div class="card-body">
                        <h5 class="card-title">Six Week Transformation</h5>
                        <p class="card-text">Ideal for breaking unwanted habits, stopping binge eating, improving
                            self-esteem, body confidence and motivation to eat healthily &amp; exercise.</p>
                        <p class="card-text">Includes a 30-minute consultation call to map out your transformation, a
                            2-hour Rapid Transformational Therapy Session, Personal Recording for 30 Days and weekly
                            follow up 30 minute coaching calls.</p>
                        <!-- <div class="price">£647</div> -->
                    </div>
                </div>
                <div class="card">
                    <img src="../img/notebook.jpg"
                        alt="Twelve Week Transformation, hypnotherapy, RTT, Samantha Goddard">
                    <div class="card-body">
                        <h5 class="card-title">Twelve Week Transformation</h5>
                        <p class="card-text">Ideal for deep transformation of negative body image, low self-esteem,
                            anxiety, guilt, depression and eating issues.</p>
                        <p class="card-text">Includes a 30-minute consultation call to map out your transformation, two
                            2-hour Rapid Transformational Therapy Sessions, Two Personal Recordings &amp; weekly follow
                            up &amp; coaching calls.</p>
                        <!-- <div class="price">£997</div> -->
                    </div>
                </div>
                <div class="card">
                    <img src="../img/phone.jpg" alt="One to One Coaching calls, hypnotherapy, RTT, Samantha Goddard">
                    <div class="card-body">
                        <h5 class="card-title">One to One Coaching calls</h5>
                        <p class="card-text">If you want a one off call, or regular coaching calls to help and support
                            you through any challenges life may have thrown you please get in touch and we’ll make it
                            happen.</p>
                        <!-- <div class="price">30 minutes £60</div> -->
                        <!-- <div class="price">1 hour £120</div> -->
                    </div>
                </div>
            </div>
            <div class="button-container">
                <a href="https://app.10to8.com/book/speaktosam/" target="_blank">
                    <button class="button">Book A Call</button>
                </a>
            </div>
        </div>
    </section>